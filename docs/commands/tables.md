...
**QUALIFIED_NAME must already exist in the endpoint's catalog.** The procedure does not create a table on demand: naming a table that is not present fails with an actionable error pointing at [`fdw sql-endpoints refresh`](sql-endpoints.md#sql-endpoints-refresh) to force an item-level sync first, rather than the command silently doing nothing. Confirmed against a live endpoint - `tables refresh` has no item-scoped side effect.

**The procedure can also decline a table by type**, reported as "Refresh is not supported for this type of table" and translated into the same actionable error as the legacy-sync case above. Microsoft's reference for this procedure does not document which table types it accepts or rejects, and we have not yet observed it succeed against a live endpoint - do not read this command's presence in the CLI as proof that a refresh has worked end to end. If it fails for you with either message, [`fdw sql-endpoints refresh`](sql-endpoints.md#sql-endpoints-refresh) is the working alternative for that table.
...
`qualified_name` must already name a table present in the endpoint's catalog: the procedure does not create one on demand. Passing a name that is not present raises a `ToolError` naming the table and pointing at `refresh_sql_endpoint_metadata` to force an item-level sync first - confirmed against a live endpoint, this tool has no item-scoped side effect.

The procedure can also decline a table by type, reported as "Refresh is not supported for this type of table" and raised as a `ToolError` in the same family as the legacy-sync message above. Microsoft's reference for this procedure does not document which table types it accepts or rejects, and we have not yet observed this tool succeed against a live endpoint - its availability in the MCP surface is not proof that a refresh has worked end to end. If it fails for you with either message, `refresh_sql_endpoint_metadata` (or `fdw sql-endpoints refresh`) is the working alternative for that table.
