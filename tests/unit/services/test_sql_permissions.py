...
class TestBuildScopeClause:
    def test_database_scope(self) -> None:
        ...
        assert result == "ON DATABASE::CURRENT"
...
class TestGrantPermission:
    async def test_grant_select_on_database_builds_correct_sql(self) -> None:
        ...
        stmt = captured[0]
        assert stmt == "GRANT SELECT ON DATABASE::CURRENT TO [user@contoso.com];"
    ...
    async def test_grant_on_schema_scope(self) -> None:
        ...
        assert "DENY" not in captured[0]
        assert "ON SCHEMA::[dbo]" in captured[0]
        assert "TO [analysts]" in captured[0]
...
class TestListSqlPermissions:
    async def test_returns_database_permission_objects(self) -> None:
        perm_rows = [
            ("alice@contoso.com", "EXTERNAL_USER", "GRANT", "SELECT", "DATABASE", 0, 0),
        ]
        obj_rows: list = []
        schema_rows: list = []
    ...
    async def test_schema_class_resolved_to_schema_name(self) -> None:
        perm_rows = [
            ("alice@contoso.com", "EXTERNAL_USER", "GRANT", "SELECT", "SCHEMA", 5, 0),
        ]
        obj_rows: list = []
        schema_rows = [(5, "dbo")]
    ...
    async def test_object_class_resolved_to_object_names(self) -> None:
        perm_rows = [
            ("alice@contoso.com", "EXTERNAL_USER", "GRANT", "SELECT", "OBJECT_OR_COLUMN", 10, 0),
        ]
    ...
    async def test_filter_by_principal(self) -> None:
        perm_rows = [
            ("alice@contoso.com", "EXTERNAL_USER", "GRANT", "SELECT", "DATABASE", 0, 0),
            ("bob@contoso.com", "EXTERNAL_USER", "GRANT", "INSERT", "DATABASE", 0, 0),
        ]
